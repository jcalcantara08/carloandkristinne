...
export function FormStatus({ state }: { state: ActionState }) {
  ...
  return (
    <div
      ...
      className={
        success
          ? "flex items-start gap-3 rounded-xl border border-brand-mauve-600/50 bg-brand-mauve-600/10 px-4 py-3 text-sm text-brand-mauve-600"
          : "flex items-start gap-3 rounded-xl border border-brand-plum-600/50 bg-brand-plum-600/10 px-4 py-3 text-sm text-brand-plum-600"
      }
    >
      {success ? (
